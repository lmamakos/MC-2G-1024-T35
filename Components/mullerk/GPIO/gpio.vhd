-- General-purpose I/O.
--
-- A simple GPIO unit for a 6809/Z80 multicomp. Aims to provide a number of
...
--
-- GPIO PROGRAMMING INTERFACE (Modified for Multicomp Z80)
-- =======================================================
--
...
-- $FFD7 GPIODAT
--
-- For Multicomp Z80 this is changed to 2 Port-Addresses:
-- $C0  GPIOADR
-- $C1  GPIODAT
--
...
-- (The shown table reflects the Multicomp configuration
--  with 1x2-Bit Port and 1x8-Bit Port !)
--
-- 0 DAT0 bits [1:0]
-- 1 DDR1 bits [1:0]
-- 2 DAT2 bits [7:0]
...
port (
        ...
        -- GPIO
        dat0_i    : in  std_logic_vector(3 downto 0);
        dat0_o    : out std_logic_vector(3 downto 0);
        n_dat0_oe : out std_logic_vector(3 downto 0);
...
architecture rtl of gpio is
  ...
  signal reg_dat0   : std_logic_vector(3 downto 0);
  signal reg_dat0_d : std_logic_vector(3 downto 0);
  signal reg_ddr0   : std_logic_vector(3 downto 0);
...
begin
  ...
  proc_dat0: process(reg_ddr0, reg_dat0, dat0_i, dataIn, reg, n_wr, regAddr)
    begin
      if reg = x"00" and n_wr = '0' and regAddr = '1' then
        -- write
        reg_dat0_d <= (dataIn(1 downto 0) and not reg_ddr0) or (dat0_i and reg_ddr0);
      else
        -- hold
  ...
  dataOut <= reg                 when regAddr = '0' else
               "0000" & reg_dat0 when regAddr = '1' and reg = "00000000" else
               "0000" & reg_ddr0 when regAddr = '1' and reg = "00000001" else
  ...
  proc_reg: process(clk, n_reset)
    begin
      if n_reset='0' then
        reg      <= "00000000";
        reg_dat0 <= "0000";
        reg_ddr0 <= "1100";      -- default for n_INTA & n_INTB as INPUT
        reg_dat2 <= "00000000";
        reg_ddr2 <= "00000000";
        ...
        if hold = '0' and n_wr = '0' then
          if regAddr = '0' then
            ...
            if reg = "00000001" then reg_ddr0 <= dataIn(3 downto 0); end if;
